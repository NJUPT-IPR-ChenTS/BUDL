...
class LeWinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8, shift_size=0,
        ...
        '''swin v2'''
        # self.attn = WindowAttention(
        #     dim, window_size=to_2tuple(self.win_size), num_heads=num_heads,
    ...
    def forward(self, x,H, W, mask=None):
        ...
        if self.cross_modulator is not None:
            ...
            x_cross = self.cross_attn(x, self.cross_modulator.weight)        #   self.cross_attn = Attention 函数
            x_cross = x_cross.view(B, H + pad_b, W + pad_r, C)
            x = shortcut + x_cross
            shortcut = shortcut[:, :H, :W, :]
            shortcut = shortcut.contiguous().view(B, H * W, C)
        ...
        x = x + self.drop_path(self.norm2(self.mlp(x,H,W)))
        del attn_mask
        return x
...
class BasicUformerLayer(nn.Module):
    ...
    def forward(self, x,X_H, X_W, mask=None):
        for blk in self.blocks:
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x)
            else:
                x = blk(x,X_H, X_W, mask)
        return x
...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
                 embed_dim=32, depths=[2, 2, 4,  2,  4, 2, 2], num_heads=[1, 2, 4, 8, 8, 4, 2],
        ...
        for i_layer in range(4):
            layer = BasicUformerLayer(dim=embed_dim * 8,
                                          output_dim=embed_dim * 8,
                                          input_resolution=(img_size // (2 ** 3),
                                                            img_size // (2 ** 3)),
                                          depth=depths[3],
    ...
    def forward(self, x, mask=None):
        ...
        y = self.pos_drop(y)
        # Encoder
        conv0 = self.encoderlayer_0(y,X_H, X_W, mask=mask)
        pool0 = self.dowsample_0(conv0,X_H, X_W)
        conv1 = self.encoderlayer_1(pool0,X_H//2, X_W//2, mask=mask)
        pool1 = self.dowsample_1(conv1,X_H//2, X_W//2)
        conv2 = self.encoderlayer_2(pool1,X_H//4, X_W//4, mask=mask)
        pool2 = self.dowsample_2(conv2,X_H//4, X_W//4)
        ...
        for layer in self.layers:
            temp = layer(temp,X_H//8, X_W//8,mask=mask)
        ...
        up1 = self.upsample_1(conv4,X_H//8, X_W//8)
        deconv1 = torch.cat([up1, conv2], -1)
        deconv1 = self.decoderlayer_1(deconv1,X_H//4, X_W//4, mask=mask)

        up2 = self.upsample_2(deconv1,X_H//4, X_W//4)
        deconv2 = torch.cat([up2, conv1], -1)
        deconv2 = self.decoderlayer_2(deconv2,X_H//2, X_W//2, mask=mask)

        up3 = self.upsample_3(deconv2,X_H//2, X_W//2)
        deconv3 = torch.cat([up3, conv0], -1)
        deconv3 = self.decoderlayer_3(deconv3,X_H, X_W, mask=mask)

        # Output Projection
        y = self.output_proj(deconv3,X_H, X_W)
        #y = pad_tensor_back(y, pad_left, pad_right, pad_top, pad_bottom)
...
if __name__ == "__main__":
    ...
    model_restoration = Uformer(img_size=input_size, embed_dim=32,
                                win_size=8, mlp_ratio=4., token_projection='linear', token_mlp='mlp', modulator=True,
                                shift_flag=True,cross_modulator=True)
    ...
    print(out.shape)
    print("Parameters of full network %.4f " % (sum([m.numel() for m in model_restoration.parameters()]) / 1e6))
