...
parser.add_argument("--experiment_name", type=str, default="SwinIR_LOL01", help="name of the dataset")

parser.add_argument("--epoch", type=int, default=0, help="epoch to start training from")
parser.add_argument("--n_epochs", type=int, default=200, help="number of epochs of training")
parser.add_argument("--dataset_name", type=str, default="D:\SwinIR_CycleGAN\LOL", help="name of the dataset")
...
G_AB = ENSwinIR(upscale=1, in_chans=3, img_size=128, window_size=8,
            img_range=1., depths=[2, 2, 2, 2], embed_dim=240, num_heads=[6, 6, 6, 6],
            mlp_ratio=2, upsampler='null', resi_connection='1conv')
G_BA = ENSwinIR(upscale=1, in_chans=3, img_size=128, window_size=8,
            img_range=1., depths=[2, 2, 2, 2], embed_dim=240, num_heads=[6, 6, 6, 6],
            mlp_ratio=2, upsampler='null', resi_connection='1conv')
...
if opt.epoch != 0:
    ...
    G_AB.load_state_dict(torch.load("saved_models/%s/%s/G_AB_%d.pth" % (opt.dataset_name, opt.experiment_name, opt.epoch)))
    G_BA.load_state_dict(torch.load("saved_models/%s/%s/G_BA_%d.pth" % (opt.dataset_name, opt.experiment_name, opt.epoch)))
    D_A.load_state_dict(torch.load("saved_models/%s/%s/D_A_%d.pth" % (opt.dataset_name, opt.experiment_name, opt.epoch)))
    D_B.load_state_dict(torch.load("saved_models/%s/%s/D_B_%d.pth" % (opt.dataset_name, opt.experiment_name, opt.epoch)))
#else:
#    # Initialize weights
...
transforms_ = [
    ...
    transforms.RandomCrop((opt.img_height, opt.img_width), Image.BICUBIC),
...
def sample_images(batches_done):
    ...
    save_image(image_grid, "images/%s/%s/%s.png" % (opt.dataset_name, opt.experiment_name, batches_done), normalize=False)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        # Identity loss
        #A身份的损失 = 生成器通过A(油画)的生成效果与A原来图片的损失
        #B身份的损失 = 生成器通过B(油画)的生成效果与B原来图片的损失
        ...
        # GAN loss
        #fake_B是真实油画图片A生成的伪现实图片
        fake_B = G_AB(real_A)
        #用判别器判别生成的伪现实图片与现实图片之间的差距
        loss_GAN_AB = criterion_GAN(D_B(fake_B), valid)#MSELoss()该统计参数是预测数据和原始数据对应点误差的平方和的均值
        #fake_A是真实油画图片B生成的伪现实图片
        fake_A = G_BA(real_B)
        #用判别器判别生成的伪油画图片与油画图片之间的差距
        loss_GAN_BA = criterion_GAN(D_A(fake_A), valid)
        #计算GAN网络的损失
        loss_GAN = (loss_GAN_AB + loss_GAN_BA) / 2

        # Cycle loss
        #通过真实油画图片生成伪现实图片，再进行一次反生成，生成伪真实图片
        recov_A = G_BA(fake_B)
        #计算伪真实图片与真实图片的损失
        loss_cycle_A = criterion_cycle(recov_A, real_A)#L1Loss  绝对值误差
        #通过真实油画图片生成伪现实图片，再进行一次反生成，生成伪真实图片
        recov_B = G_AB(fake_A)
        #计算伪真实图片与真实图片的损失
