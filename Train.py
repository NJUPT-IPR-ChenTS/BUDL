...
from models import lossfn
from models.FDSP import *
...
G_AB = BUDL(upscale=1, in_chans=3, img_size=256, window_size=8,
            ...
            mlp_ratio=2, upsampler='null', resi_connection='1conv', train=True, withzero=False)
G_BA = BUDL(upscale=1, in_chans=3, img_size=256, window_size=8,
            img_range=1., depths=[2, 2, 2, 2], embed_dim=64, num_heads=[6, 6, 6, 6],
            mlp_ratio=2, upsampler='null', resi_connection='1conv', train=True, withzero=True)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for j, batch in enumerate(dataloader):
        ...
        loss_BDSP_A = criterion_BDSP(FDSP(fake_B), FDSP(real_A))
        loss_BDSP_B = criterion_BDSP(FDSP(fake_A), FDSP(real_B))
        ...
        loss_BDSP_RA = criterion_BDSP(FDSP(recov_A), FDSP(real_A))
        loss_BDSP_RB = criterion_BDSP(FDSP(recov_B), FDSP(real_B))
