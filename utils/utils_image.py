...
def imread_uint_hsv(path, n_channels=3):
    #  input: path
    # output: HxWx3(RGB or GGG), or HxWx1 (G)
    if n_channels == 1:
        img = cv2.imread(path, 0)  # cv2.IMREAD_GRAYSCALE
...
def tensor2uint_BN(img):
    image_numpy = img.data.squeeze().float().cpu().numpy()
    image_numpy = (np.transpose(image_numpy, (1, 2, 0)) + 1) / 2.0 * 255.0
    image_numpy = np.maximum(image_numpy, 0)
    image_numpy = np.minimum(image_numpy, 255).astype(np.uint8)
    return image_numpy

def tensor2uint_BNN(img):
    image_numpy = img.data.squeeze().float().cpu().numpy()
    image_numpy = (np.transpose(image_numpy, (1, 2, 0)) + 1) / 2.0 
    image_numpy = ((image_numpy - image_numpy.min()) / (image_numpy.max() - image_numpy.min()) * 255.0).astype(np.uint8)
    return image_numpy
